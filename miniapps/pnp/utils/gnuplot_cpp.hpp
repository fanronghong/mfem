 ...
 *
 * Requirements:
 * 1. gnuplot has to be installed (http://www.gnuplot.info/download.html)
 * 2. for Windows: set Path-Variable for Gnuplot path
...
#ifndef LEARN_MFEM_GNUPLOT_CPP_HPP
#define LEARN_MFEM_GNUPLOT_CPP_HPP
...
public:
    ...
    Gnuplot& cmd(const std::string &cmdstr); // 一次接受一组gnuplot命令
    // ---------------------------------------------------------------------------------
...
{
    /* 里面有一个关键函数 showonscreen(),
     * 里面又会执行两个命令:
     *  cmd("set output"): set output to redirect output to a file
     *  cmd("set terminal " + Gnuplot::terminal_std): set terminal to select a printer
     */
    init();
...
Gnuplot& Gnuplot::plot_xy(const X& x, const Y& y, const std::string &title)
{
    ...
    tmp.flush();
    tmp.close(); // tmp 里面就是要画图的数据, 比如两列数据(横坐标x, 纵坐标y). 可以查看tmp文件
...
{
    std::ostringstream cmdstr;
    cmdstr << "set pointsize " << pointsize; // http://www.gnuplotting.org/plotting-data/
    cmd(cmdstr.str());
                              ...
                              const std::string &title)
{
    ...
    // filename就是存放数据的文件名, column_x就是数据中的列指标, column_y表示数据文件中的列指标
    ...
    // Do the actual plot fffffffffffff
...
{
    if( !(valid) ) // validation of gnuplot session
    {
        return *this;
    }
...
void Gnuplot::init()
{
...
#elif defined(unix) || defined(__unix) || defined(__unix__) || defined(__APPLE__)
    gnucmd = popen(tmp.c_str(),"w"); // fffffffffffff
#endif
    ...
    nplots = 0; // number of plots in session
    valid = true; // validation of gnuplot session
    smooth = ""; // interpolate and approximate data in defined styles (e.g. spline)

    //set terminal type fffffffffff
    showonscreen();
...
#endif //LEARN_MFEM_GNUPLOT_CPP_HPP