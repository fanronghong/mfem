...
#include <iostream>
#include "gnuplot_cpp.hpp" //Gnuplot class handles POSIX-Pipe-communikation with Gnuplot
#include "mfem.hpp"
...
void plot2()
{
    ...
    {
        x.push_back(i * 2.0 * M_PI /  size);
        y.push_back(cos(i * 2.0 * M_PI /  size));
    }

    cout << "window 2: user defined points" << endl;
    Gnuplot g;
    g.plot_xy(x, y,"points");
//    g2.set_smooth().plot_x(y,"cspline");
//    g2.set_smooth("bezier").plot_x(y,"bezier");
//    g2.unset_smooth();
    wait_for_key();
}
void plot3()
{
    ...
    g.set_grid();
    g.plot_equation("log(x)/x","log(x)/x");
    wait_for_key();
}
void plot4()
{
    ...
    g.set_zrange(0,100);
    g.set_xlabel("x-axis").set_ylabel("y-axis").set_zlabel("z-axis");
    g.plot_equation3d("x*x+y*y");
    wait_for_key();
}
void plot5()
{
    ...
    g.set_isosamples(25).set_hidden3d();
    g.plot_equation3d("x*y*y");
    wait_for_key();
}
void plot6()
{
    ...
    g.set_isosamples(60).set_contour();
    g.unset_surface().plot_equation3d("sin(x)*sin(y)+4");
    g.set_surface().replot();
    wait_for_key();
}
void plot7()
{
    ...
    g.set_xrange(-30,20).set_samples(40);
    g.plot_equation("besj0(x)*0.12e1").plot_equation("(x**besj0(x))-2.5");
    g.set_samples(400).replot();
    wait_for_key();
}
void plot8()
{
    Gnuplot g("filledcurves");
    cout << "window 8: filledcurves" << endl;
    g.set_legend("outside right top").set_xrange(-5,5);
    g.plot_equation("x*x").plot_equation("-x*x+4");
    wait_for_key();
}
void plot9()
{
    ...
    g.set_pointsize(0.6).unset_legend().plot_slope(0.8,20);
...
void plot10()
{
    ...
    g.cmd("set samples 400").cmd("plot abs(x)/2"); // either with cmd()
    g << "replot sqrt(x)" << "replot sqrt(-x)";    // or with <<
    wait_for_key();
}
...
//        plot10();
    ...
    catch (GnuplotException ge) // Using the GnuplotException class
    {
        cout << ge.what() << endl;
    }
